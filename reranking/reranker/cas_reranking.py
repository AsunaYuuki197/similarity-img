...
def sparse_feature_construction(all_num, original_dist, initial_rank, 
    ...
    confident_matrix = confident_candidate_expansion(all_num, initial_rank, k=confident_k, device=device)
    candidate_matrix = candidate_expansion(all_num, initial_rank, k=candidate_k, device=device)
    ...
    F_matrix = similarity_diffusion(all_num, confident_matrix, similarity_matrix, initial_rank, knn=confident_k, lambda_value=lambda_value, mu=mu, device=device) # R-GeM
    end_time = time.time()
    print(f'===> Time cost {end_time-start_time}')
...
def cas_reranking(query_features, 
                    ...
                    device=torch.device('cuda'),
                    mask=None):
    """Computes the reranking distance. Step-by-step:
    1. Building a kNN graph of query and gallery features.
    2. Diffusing similarity across this graph (locally and globally).
    3. Computing new distances using both local weights and shortest-paths.
    4. Fusing original and refined distances to improve retrieval results.   

    Args:
        query_features (torch.Tensor): N-D or (D,) feature matrix.
        gallery_features (torch.Tensor): M-D feature matrix.

    Returns:
        torch.Tensor: reranking distance matrix (N, M).
    """
    ...
    sparse_feature, confident_matrix, candidate_matrix, F_matrix, momentum = sparse_feature_construction(all_num, original_dist, initial_rank, confident_k=k1, candidate_k=k2, trans_k=k3, beta=beta, sigma=sigma, mu=mu, device=device)
    
    refine_cost = torch.ones(all_num, all_num)
    refine_cost[torch.arange(all_num), torch.arange(all_num)] = 0
    sparse_feature = markov_expansion(all_num, sparse_feature, initial_rank, k1=k4, k2=k5, confident_matrix=confident_matrix, lambda_value=lv, device=device)

    ## method 1: Direct Jensen-Shannon Distance - too long in CPU because 'jensen shannon' divergence (242.5656476020813 secs)
    # modified_dist = compute_distance_matrix(sparse_feature, sparse_feature, 'jensen shannon', device=device)
   

    ## method 2: Graph-based shortest path over kNN (768.3858647346497 secs)
    adjacency_matrix = 50*torch.ones((all_num, all_num)).to(device)
    modified_dist = compute_distance_matrix(sparse_feature, sparse_feature, 'jensen shannon', device=device)
    # Only keep JS distances between top-100 nearest neighbors
    ...
    # Apply shortest-path algorithm (graph diffusion) (O(n^3)) - too long in CPU
    modified_dist = all_pairs_shortest_paths(adjacency_matrix, max_threshold=50, device=device)


    ## method 3: Cosine distance
    # modified_dist = compute_distance_matrix(sparse_feature, sparse_feature, 'cosine').to(device)


    ## method 4: Euclid distance
    # modified_dist = compute_distance_matrix(sparse_feature, sparse_feature, 'euclidean').to(device)
