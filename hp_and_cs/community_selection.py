...
def prepare_community_selection(root, dataset, COMMUNITY_SELECTION):  
    
    graph_directaries={'roxford':'graph/delg/roxford/0301/',
                 'roxford5k':'graph/delg/roxford/0301/',
    ...
    try:
        graph_dir= root + '/' + graph_directaries[_dataset]
    except:
    ...
    if _dataset.startswith('roxford'):
        _dataset = 'roxford'
        with open(f'{root}/data/roxford/gnd_roxford5k.pkl','rb') as f:
            _dataset_meta=pickle.load(f)
    elif _dataset.startswith('rparis'):
        _dataset = 'rparis'
        with open(f'{root}/data/rparis/gnd_rparis6k.pkl','rb') as f:
            _dataset_meta=pickle.load(f)
    ...
    if COMMUNITY_SELECTION==2:
        ...
        def _read_delg_index():
            ...
            for img in _dataset_meta['imlist']:
                geom_path= root + '/' + 'features/'+_dataset+'_np_delg_features/'+img+'_local_locations.npy'
                features_path= root + '/' + 'features/'+_dataset+'_np_delg_features/'+img+'_local_descriptors.npy'
...
def find_dominant(Gs,first_search,q):
    ...
    for i in first_search[:100]:
        ...
        img1 = cv2.imread('dataset/roxford5k/query_jpg/'+_dataset_meta['qimlist'][q]+'.jpg',cv2.IMREAD_COLOR)  
