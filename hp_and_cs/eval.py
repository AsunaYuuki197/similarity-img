...
COMMUNITY_SELECTION=2# only calculate the uncertainty score, but do not apply geometric verification
...
import dataset
from utils.evaluate import compute_map, compute_map_and_print
...
for q in range(70):
    ...
    total_times.append(time.time() - start_q)
...
# print(hard_metrics)
compute_map_and_print(dataset_name, total_ranks_.T, ground_truth)
print("All dataset Evaluate Time (seconds)", time.time() - start)
print("Each Query Avg Executed Time (seconds)", sum(total_times)/len(total_times))
