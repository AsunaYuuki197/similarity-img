...
def MatchFeatures(query_locations,
  ...
  query_inlier_locations=[query_locations_to_use[i] for i,x in enumerate(inliers) if x !=0]
  index_inlier_locations=[index_image_locations_to_use[i] for i,x in enumerate(inliers) if x !=0]
  
  #return the matched local feature indexes. q2i (query_descriptor_index, img_descriptor_index)
  true_indices_index=[i for i in range(num_features_query) if indices[i] != num_features_index_image]
  q2i=np.array([ (true_indices_index[i],indices[true_indices_index[i]]) for i,x in enumerate(inliers) if x !=0])
  i2q=np.array([ (indices[true_indices_index[i]],true_indices_index[i]) for i,x in enumerate(inliers) if x !=0])
  
  return sum(inliers), match_viz_bytes,query_inlier_locations,index_inlier_locations,q2i,i2q
